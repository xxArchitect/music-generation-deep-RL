...
class ScaleDegreeTuner(RLTuner):
    """RLTuner variant that emphasizes important scale degrees."""
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.scale_degree_scaler = 0.3
        self.SCALE_DEGREES = {
            0: 'TONIC',
            2: 'SUPERTONIC',
            4: 'MEDIANT',
    ...
    def reward_music_theory(self, action):
        reward = super().reward_music_theory(action)
        reward += self.reward_scale_degrees(action)
        return reward
        
    def reward_scale_degrees(self, action):
        current_note = np.argmax(action)
        
        if current_note in (NOTE_OFF, NO_EVENT):
            return 0.0
            
        scale_position = current_note % 12
        if scale_position not in self.SCALE_DEGREES:
            return 0.0
            
        degree = self.SCALE_DEGREES[scale_position]
        if degree in ['TONIC', 'DOMINANT', 'LEADING_TONE']:
            return self.scale_degree_scaler * 0.3
        return self.scale_degree_scaler * 0.1